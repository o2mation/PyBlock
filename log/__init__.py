...
class BlockRootLogger(RootLogger):

    def __init__(self, level):
        super(BlockRootLogger, self).__init__(level)
    ...
    def error(self, msg, *args, **kwargs):
        ...
        if _t == "None\n":
            ...
            msg = _t[:-1]
        super(BlockRootLogger, self).error(msg, *args, **kwargs)
        if isPublished:
            self.senderror(msg)
...
def handle_exception(exc_type, exc_value, exc_traceback):
    ...
    BlockRootLogger.senderror(repr("Uncaught exception %s" % info))

logging.root = BlockRootLogger(logging.WARNING)
